    ...
    <div class="card-body">
      <table width="100%">
        ...
        <tr>
          ...
          <td width="1%"></td>
          <td class="pt-5">
            <a href="{{ route('admin.verifikasi-pegawai.accept', ['id' => $user->id]) }}" onclick="return confirm('Apakah anda yakin akan menerima data pegawai ini?')" class="btn btn-success"><i class="fas fa-check mr-2"></i> {{ __('Terima') }}</a>
            <a href="{{ route('admin.verifikasi-pegawai.reject', ['id' => $user->id]) }}" onclick="return confirm('Apakah anda yakin akan menolak data pegawai ini?')" class="btn btn-danger"><i class="fas fa-times mr-2"></i> {{ __('Tolak') }}</a>
          </td>
        </tr>
      </table>
    </div>
