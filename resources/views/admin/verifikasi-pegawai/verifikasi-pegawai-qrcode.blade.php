    ...
    <div class="col-md-6">
      <div class="card shadow mb-4">
        <div class="card-header py-3">
          <h6 class="m-0 font-weight-bold text-primary">Cetak Tanda Tangan Elektronik - {{ $user->nama }}</h6>
        </div>
        <div class="card-body">
              ...
              <div class="modal fade text-left" id="modalUploadLogo" tabindex="-1" role="dialog" aria-labelledby="myModalLabel1"
                ...
                <div class="modal-dialog " role="document">
                  <div class="modal-content">
                    ...
                    <form id="form-upload-logo" action="{{ route('admin.pegawai.upload-logo', ['id' => $user_id]) }}" method="POST">
                      @csrf
          ...
          <form id="form-generate-qrcode" action="{{ route('admin.pegawai.generate-qrcode') }}" method="POST">
            @csrf
...
$(document).ready(function() {
  $('#form-generate-qrcode').submit(function(e) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          url: "{{ route('admin.pegawai.generate-qrcode') }}",
          type: "POST",
  ...
  $(document).on('click', '#download', function() {
    html2canvas(document.getElementById("qrcode-result")).then(function(canvas){
      ...
      $.ajax({
        url: "{{ route('admin.verifikasi-pegawai.sendToMail') }}",
        type: "POST",
        data: {
          _token: '{{ csrf_token() }}',
          img: img,
